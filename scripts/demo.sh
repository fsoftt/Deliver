...
# eventually <description> <timeout-seconds> <command...>: retries until the command succeeds.
eventually() {
...
deliver_shipment() {
  eventually "shipment $1 assigned to a driver" 30 test "$(shipment_field "$1" .status)" = Assigned
  api -X POST "$GATEWAY/api/shipments/$1/pickup"  && ok "picked up"
  api -X POST "$GATEWAY/api/shipments/$1/transit" && ok "in transit"
  api -X POST "$GATEWAY/api/shipments/$1/deliver" && ok "delivered"
}
...
eventually "customer notified about Carlos" 30 bash -c "curl -fsS '$GATEWAY/api/notifications?shipmentId=$SHIPMENT_ID' | grep -q 'driver Carlos'"
...
eventually "notification delivered after retries" 40 bash -c "curl -fsS '$GATEWAY/api/notifications?shipmentId=$FLAKY_ID' | grep -q 'We received your shipment'"
