...
billing.MapGet("/{shipmentId:guid}", async Task<Results<Ok<BillingSummary>, NotFound>> (
        Guid shipmentId, IInvoiceReadStore store, CancellationToken ct) =>
        await store.GetByShipmentAsync(shipmentId, ct) is { } invoice ? TypedResults.Ok(invoice.ToSummary()) : TypedResults.NotFound())
    .WithName("GetBillingSummary");

billing.MapGet("/{shipmentId:guid}/invoice", async Task<Results<Ok<InvoiceDetails>, NotFound>> (
        Guid shipmentId, IInvoiceReadStore store, CancellationToken ct) =>
        await store.GetByShipmentAsync(shipmentId, ct) is { } invoice ? TypedResults.Ok(invoice) : TypedResults.NotFound())
    .WithName("GetInvoice");
